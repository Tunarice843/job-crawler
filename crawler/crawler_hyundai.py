...
# -*- coding: utf-8 -*-
"""
현대차 채용공고 크롤러 (JSON API 직접 호출, 페이지네이션 지원)
- 출처: https://talent.hyundai.com/api/rec/AP-HM-FO-02700  (X-HKMC-SERVICE: HM)
...
def crawl(config):
    """현대차 채용 API를 호출해 검색어에 맞는 공고만 반환."""
    print(f"\n🚗 {COMPANY} 크롤링 시작...")
    jobs = []

    url = "https://talent.hyundai.com/api/rec/AP-HM-FO-02700"
    headers = {
        "Accept": "application/json, text/plain, */*",
        "Accept-Language": "ko,en;q=0.9,en-US;q=0.8",
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
        "Referer": "https://talent.hyundai.com/apply/applyList.hc",
        "X-HKMC-SERVICE": "HM",
        "X-HKMC-TOKEN": "null",
    }
...
def _extract_list(result):
    """현대 응답에서 공고 리스트를 꺼냄 (응답 구조 변동 대비)."""
...
def _parse_job(job):
    """현대 API 응답 1건 → 표준 공고 dict. 실패 시 None."""
    title = (
    ...
    posted_date = (
        ...
        or _parse_compact_date(job.get("opsStaDt"))
